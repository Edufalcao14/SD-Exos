...
public class EquipeGuerriers {
	...
	public Guerrier jouer(int pointsDeViePerdus){
		if (listeGuerriersEnJeu.isEmpty()) throw new NoSuchElementException("Tous les guerriers sont morts");
		Guerrier guerrier = listeGuerriersEnJeu.getFirst();
		guerrier.setPointsDeVie(guerrier.getPointsDeVie()-pointsDeViePerdus);
		listeGuerriersEnJeu.remove(1);
	...
	public int ajouterNouveauGuerrier(){
		...
		vecteurGuerriers.add(guerrier);
		int numeroGuerrier = guerrier.getNumero();
		if (listeGuerriersEnJeu.isEmpty()){
			listeGuerriersEnJeu.addFirst(guerrier);
			return numeroGuerrier;
		}
		int index = 0;
		int indiceMax = 0;
		for (Guerrier guerriers : listeGuerriersEnJeu) {
			if (guerriers.getNumero()>numeroMax){
				numeroMax=guerriers.getNumero();
				indiceMax = index;
			}
			index++;
		}
		listeGuerriersEnJeu.add(indiceMax+1,guerrier);
		return numeroGuerrier;

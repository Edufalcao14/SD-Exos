...
public class ListeRecCaracteres {
    ...
    private int nombreOccurrences(NoeudCaractere noeud, char caractereRecherche) {
        ...
        if (noeud.caractere == caractereRecherche) return 1 + nombreOccurrences(noeud.suivant, caractereRecherche);
    ...
    private char max(NoeudCaractere noeud){
        if (noeud==null) return ' ';
        char maxRec = max(noeud.suivant);
        if (noeud.caractere>maxRec){
    ...
    public ArrayList<Character> enArrayList() {
        ...
        toArrayList(tete,arrayList);
        return arrayList;

    }
      public void toArrayList(NoeudCaractere noeudCaractere , ArrayList<Character> array) {
        if (noeudCaractere==null) return;
        array.add(noeudCaractere.caractere);
        toArrayList(noeudCaractere.suivant,array);
    }
    ...
    private boolean estEgalA(NoeudCaractere noeud,ListeRecCaracteres l) {
        ...
        if (!l.contient(noeud.caractere)) return false;

        if (l.supprimerPremiereOccurrence(noeud.caractere)){
            return true;
        }
        return true;
    }

...
public class EnsembleTrieImpl<E> implements EnsembleTrie<E> {
	...
	private boolean contient(E element , Noeud noeud){
		if (noeud == null) return false;
		if(((Comparable<E>)element).compareTo(noeud.element)==0) return true;

		return contient(element,noeud.gauche) || contient(element,noeud.droit);
	}
	...
	public E predecesseur(E element) {
		if (!this.contient(element)) return null;
		if (this.min().equals(element)) return null;

		ArrayList<E> liste = new ArrayList<>();
		remplirListe(racine,liste);

		int indexElement=liste.indexOf(element)-1;

...
public class ClasseEtudiants{
	...
	public ClasseEtudiants(int nombreSeries,String[] noms){
		...
		this.tableSeries = new SerieEtudiants[nombreSeries];
		int index = 0;
		for (int i = 0; i < nombreSeries; i++) {
			SerieEtudiants serie = new SerieEtudiants(i+1);
			this.tableSeries[i] = serie;
		}
		for (String nom :noms) {
			SerieEtudiants serieEtudiants = this.tableSeries[index];
			serieEtudiants.ajouterEtudiant(nom);
			index= (index+1) % nombreSeries;
		}
	}
	...
	public SerieEtudiants getSerieEtudiants(int numeroSerie)throws IllegalArgumentException{
		if (numeroSerie<0) throw new IllegalArgumentException("");
	...
	public int numeroSerie(String nom){
		...
		if (tableSeries[1].contientEtudiant(nom)) return 1;
	...
	public boolean changerSerie(String nom, int nouveauNumeroSerie){
		if (nom.equals("")||nom==null) throw new IllegalArgumentException("");
		if (nouveauNumeroSerie<1 || nouveauNumeroSerie > tableSeries.length) throw new IllegalArgumentException("");
		int numeroSerieActuel = numeroSerie(nom);
		if (numeroSerieActuel ==1 || numeroSerieActuel == nouveauNumeroSerie)
			return false;
		tableSeries[numeroSerieActuel].supprimerEtudiant(nom);
		tableSeries[nouveauNumeroSerie-1].ajouterEtudiant(nom);
		return true;
	}
		
	public String toString(){
		String toString = " Les series : " + "\n" ;
		for (int i = 0; i < tableSeries.length; i++) {
			toString+= tableSeries[i]  + "\n";
		}
